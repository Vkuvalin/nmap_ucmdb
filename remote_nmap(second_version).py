...
def dictionaryFilling(list_ip, list_id):
    for index in range(len(list_ip)):
        ip_dict[list_ip[index].replace('.', '')] = [list_ip[index], list_id[index]]
...
def performNmapDiscover(client, list_ip, configuration_file, async_nmap_script_name, timeout, scanKnownPortsOnly, portstoscan, doServiceFingerprints, discoverUdpPorts, count_stream, nmapLocation=None):
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # if nmapLocation:
    #     nmapLocation = '"' + nmapLocation + '"'.replace(' ', '')
    # else:
    #     nmapLocation = nmap.NMAP_EXECUTABLES[0].replace(' ', '')
    # command = ' '.join([nmapLocation] + parametersList)
    # ------------------------------------------------------------------------------------------------------------------

    command = ' '.join(['nmap.exe'] + parametersList)

    if not shell.isWinOs():
        command = "sudo " + command

    output = client.executeCmd("echo {} >> {}".format(count_stream, configuration_file), timeout)
    output = client.executeCmd("echo {} >> {}".format(command, configuration_file), timeout)

    for ip in list_ip:
        output = client.executeCmd("echo {} >> {}".format(ip, configuration_file), timeout)

    # logger.debug("python.exe {}".format(async_nmap_script_name))
    # output = client.executeCmd("python.exe {}".format(async_nmap_script_name))
    # output = shell.execCmd("python {}".format(async_nmap_script_name))
    # output = shell.execCmd("C:\Python27\python.exe {}".format(async_nmap_script_name))

    cmd1 = 'C:\Python27\python.exe D:\\UCMDB\\DataFlowProbe\\bin\\..\\runtime\\probeManager\\discoveryResources\\nmap_async.py'
    output = shell.execCmd(cmd1, timeout)
        ...
        if (hostnames is not None) and netutils.isValidIp(ip):
            ...
            if os and discoverOsName:
                ...
                if osClass:
                    ...
                    if not hostClass:
                        Framework.reportWarning("Unknown OS detected - '%s'. Vendor '%s', family '%s'" % (ip, osVendor, osFamily))
                        hostClass = "host"
...
def DiscoveryMain(Framework):
    ...
    count_stream = Framework.getParameter('count_stream')
    ...
    try:
        ...
        try:
            configuration_file_name = agent_ext_dir + 'configuration_nmap.txt'
            async_nmap_script_name = agent_ext_dir + 'nmap_async.py'
            file_names = agent_ext_dir + 'names_files.txt'


            performNmapDiscover(client, ip_addresses, configuration_file_name, async_nmap_script_name, timeout, scanKnownPortsOnly, portstoscan, doServiceFingerprints, discoverUdpPorts, count_stream, nmapLocation)
            process_flag = False
            names_files = list()
            while not process_flag:
                output = client.executeCmd('dir {} /s | find /i "names_files"'.format(agent_ext_dir), timeout)
                if output.find('names_files.txt') != -1:
                    process_flag = True
                    output = client.executeCmd("type {} | more".format(file_names), timeout)

                    pattern = r"\d+nmap.xml"
                    results = re.finditer(pattern, output)
                    for result in results:
                        names_files.append(agent_ext_dir + result.group(0))
                else:
                    time.sleep(60)

            for name in names_files:
                pattern = r"\d+"
                results = re.finditer(pattern, name)
                ip = [result.group(0) for result in results][0]

                processNmapResult(name, OSHVResult, discoverOsName, doServiceFingerprints, ip_dict[ip][1], Framework)
                File(name).delete()
        finally:
            File(configuration_file_name).delete()
            File(file_names).delete()
            client.close()
